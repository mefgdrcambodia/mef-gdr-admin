...
// ------------------- Main Component: FormVideoAlbum -------------------
function FormVideoAlbum({ auth, onSuccess, onCancel }) {
  const { id } = useParams();
  ...
  const [videoPreview, setVideoPreview] = useState(null); // for preview (object URL)
  ...
  const loadData = async () => {
    setIsLoading(true);
    const api = `${process.env.REACT_APP_API_HOST}/api/${process.env.REACT_APP_VERSION}/admin/event/video-album/${id}`;
    ...
    if (result.success) {
      ...
      setExistingVideo(data.videos || null);
      setVideoPreview(data.videos || null);
      setStatus(data.status === true || data.status === "true");
    }
    setIsLoading(false);
  };

  // Title image handlers (keep 5MB limit)
  ...
  // Video handlers – unlimited size (only type check)
  const handleVideoChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      if (!file.type.startsWith("video/")) {
        swalToast.toastError("សូមជ្រើសរើសឯកសារវីដេអូ", 1500);
        return;
      }
      // No size limit – unlimited video upload
      setVideoFile(file);
  ...
  const handleSubmit = async (e) => {
    ...
    else if (existingVideo) formData.append("existing_video", existingVideo);
    ...
    swalHelper
      ...
      .then(async (res) => {
        if (res) {
          setIsLoading(true);
          const api = `${process.env.REACT_APP_API_HOST}/api/${process.env.REACT_APP_VERSION}/admin/event/video-album/${process.env.REACT_APP_API_CREATE}`;
    ...
    <div>
      <form onSubmit={handleSubmit}>
        ...
        <div className="container defualt_White_Shadow_Theme">
          ...
          <div className="row">
            <div className="col-md-12">
              <Title mode={mode} title="រូបភាព និងវីដេអូ" icons={<FaVideo />} />
            </div>
          </div>
          ...
          <div className="row mb-3">
            <div className="col-md-12">
              ...
              >
                ...
                >
                  <FaVideo /> វីដេអូ (Video) *ជ្រើសរើសតែមួយ
                </label>
                <input
                  type="file"
                  accept="video/*"
                ...
                />
                <small>
                  ជ្រើសរើសវីដេអូ (MP4, MOV, AVI, etc.) - គ្មានដែនកំណត់ទំហំ
                </small>
                {(videoPreview || existingVideo) && (
                  <div className="mt-3">
                    <strong>វីដេអូបច្ចុប្បន្ន:</strong>
                    <div
                      style={{
                        position: "relative",
                        width: "100%",
          ...
          <div className="row mb-3">
            <div className="col-md-12">
              <div className="form-check form-switch">
                ...
                >
                  {status ? "បានផ្សព្វផ្សាយ (Published)" : "រក្សាព្រាង (Draft)"}
                </label>
              </div>
            </div>
          </div>
