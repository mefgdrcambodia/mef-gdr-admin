...
function IndexNews({ auth }) {
  ...
  function actionButtonDataTable(e) {
    if (e.action == "create") {
      navigate(routeScript.route().news_create.url);
    } else if (e.action == "edit") {
      navigate(
        `${routeScript.route().news_edit.url.replace(":id", e.data._id)}`,
      );
      ...
      confirmDelete(
        ...
        async () => {
          ...
          try {
            ...
            const api = `${process.env.REACT_APP_API_HOST}/api/${process.env.REACT_APP_VERSION}/admin/event/news/${process.env.REACT_APP_API_CREATE_NO_SLASH}/delete/${e.data._id}`;
