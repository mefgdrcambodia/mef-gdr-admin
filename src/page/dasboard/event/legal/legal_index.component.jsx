...
function IndexNews({ auth }) {
  ...
  function categoryCheck(row) {
    ...
    if (row.category == "law") {
      text = "ច្បាប់";
    } else if (row.category == "regulation") {
      text = "បទប្បញ្ញត្តិ";
    } else if (row.category == "decree") {
      text = "អនុក្រឹត្យ";
    } else if (row.category == "proclamation") {
      text = "សេចក្តីប្រកាស";
    } else if (row.category == "directive") {
      text = "សេចក្តីណែនាំ";
  ...
  function actionButtonDataTable(e) {
    if (e.action == "create") {
      navigate(routeScript.route().legal_create.url);
    } else if (e.action == "edit") {
      navigate(
        `${routeScript.route().legal_edit.url.replace(":id", e.data._id)}`,
      );
    } else if (e.action == "view") {
      navigate(
        `${routeScript.route().legal_view.url.replace(":id", e.data._id)}`,
      );
      ...
      confirmDelete(
        ...
        async () => {
          ...
          try {
            ...
            const api = `${process.env.REACT_APP_API_HOST}/api/${process.env.REACT_APP_VERSION}/admin/legal/${process.env.REACT_APP_API_CREATE_NO_SLASH}/${e.data._id}`;
      ...
      <div className="container defualt_White_Shadow_Theme">
        ...
        <div className="row">
          <div className="col-md-12">
            <Title mode={"list"} title={"លិខិតបទដ្ឋានគតិយុត្ត"} />
          </div>
        </div>
        <div className="row">
          <div className="col-md-12">
