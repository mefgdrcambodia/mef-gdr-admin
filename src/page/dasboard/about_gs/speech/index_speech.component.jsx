...
import {
  formatDateToKhmer,
  formatUTCToKhmer,
} from "../../../../util/khmerDateFormatter.js";
// Component
...
function IndexSpeech({ auth }) {
  ...
  // Backend
  const api = `${process.env.REACT_APP_API_HOST}/api/${process.env.REACT_APP_VERSION}/admin/about-gs/speech`;
  ...
  const [columns, setcolumns] = useState([
    {
      ...
      name: "កាលបរិច្ឆេទ",
      selector: (row) => {
        formatDateToKhmer(row.created_date, "full");
      },
      sortable: true,
      width: "200px", // Fixed width for date column
      style: {
        whiteSpace: "normal",
...

    {
      ...
      omit: false,
      name: (
        <label style={{ textAlign: "center", width: "170px" }}>រូបភាព</label>
      ),
      width: "170px",
      selector: (row) => row.title_image, // Simple selector
      sortable: false,
      cell: (row) => (
  ...
  function actionButtonDataTable(e) {
    if (e.action == "create") {
      navigate(routeScript.route().speech_create.url);
    } else if (e.action == "edit") {
      navigate(
        `${routeScript.route().speech_edit.url.replace(":id", e.data._id)}`,
      );
    } else if (e.action == "view") {
      navigate(
        `${routeScript.route().speech_view.url.replace(":id", e.data._id)}`,
      );
      ...
      confirmDelete(
        ...
        async () => {
          ...
          try {
            ...
            const api = `${process.env.REACT_APP_API_HOST}/api/${process.env.REACT_APP_VERSION}/admin/about-gs/speech/${process.env.REACT_APP_API_CREATE_NO_SLASH}/delete/${e.data._id}`;
